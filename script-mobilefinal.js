...
.desktop-nav .nav-link{font-weight:600;font-size:1rem}
.desktop-nav .nav-link:hover,.mobile-menu-panel a:hover{color:var(--accent-gold)}
...
.mobile-menu-panel{
  ...
  width:min(88vw,320px);background:#fff;border:1px solid var(--line);
  border-radius:18px;box-shadow:var(--shadow);padding:.8rem;
  display:flex;flex-direction:column;gap:.55rem;
}
.mobile-menu-panel a{
  display:block;padding:.85rem 1rem;border-radius:12px;background:#fff7f7;font-weight:600
}
...
.hero-subtitle{
  ...
  font-size:.95rem;font-weight:700;letter-spacing:.5px;color:#ffd9dc;
...
.hero-point{
  padding:.85rem;border:1px solid rgba(255,255,255,.15);
  background:rgba(255,255,255,.12);border-radius:18px;font-weight:700
}
.hero-buttons{display:flex;gap:1rem;justify-content:center;flex-wrap:wrap}
.btn{
  display:inline-block;padding:1rem 2rem;border-radius:50px;font-weight:700;
  text-transform:uppercase;border:none;cursor:pointer;text-align:center
}
...
.quick-info{
  background:var(--dark-cream);padding:1rem 1.25rem;
  border-top:1px solid rgba(212,165,169,.4);border-bottom:1px solid rgba(212,165,169,.4)
}
...
.page-hero h1,.section-title{
  font-family:'Playfair Display',serif;font-size:3rem;line-height:1.15;margin-bottom:1rem;
  background:linear-gradient(135deg,var(--charcoal),#4A3A3B);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent
}
...
.sticky-booking{
  position:fixed;left:0;right:0;bottom:0;z-index:2500;background:rgba(255,255,255,.98);
  backdrop-filter:blur(12px);border-top:1px solid var(--line);box-shadow:0 -8px 30px rgba(0,0,0,.08)
}
...
.wa-float{
  position:fixed;right:24px;bottom:88px;width:70px;height:70px;border-radius:50%;
  display:flex;align-items:center;justify-content:center;font-size:2rem;
  background:linear-gradient(135deg,var(--rose-gold),var(--accent-gold));
  box-shadow:0 15px 35px rgba(232,180,184,.4);z-index:2600
}
...
@media (max-width: 768px){
  ...
  .sticky-booking,.wa-float{display:none !important}
